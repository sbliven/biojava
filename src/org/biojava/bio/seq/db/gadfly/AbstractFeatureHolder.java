 ...
 * <p>
 * This class shouldn't be needed if I actually get my *** in
 * gear and implement a FeatureHolderFactory class.  I really should...
 * 
    ...
    FeatureHolder
{
  ...
  public FeatureHolder filter(FeatureFilter ff, boolean recurse) {
    GFFeatureHolder res = createFeatureHolder();
    for(Iterator f = features(); f.hasNext();) {
      GFFeature feat = (GFFeature) f.next();
      if(ff.accept(feat)) {
        try {
          res.addFeature(feat);
      ...
      if(recurse) {
        ...
        for(Iterator rf = r.features(); rf.hasNext();) {
          try {
            res.addFeature((GFFeature) rf.next());
          } catch (ChangeVetoException cve) {
            throw new BioError(
              cve,
  ...
  protected abstract GFFeatureHolder createFeatureHolder();
}
