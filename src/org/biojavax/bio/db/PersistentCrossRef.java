...
public abstract class PersistentCrossRef extends SimpleCrossRef implements Persistent {
    ...
    protected PersistentCrossRef(PersistentBioDB db, CrossRef cr) {
        ...
        try {
            ...
            for (int i = 0; i < terms.size(); i++) this.setTerm((ComparableTerm)terms.get(i),i);
    ...
    protected void resetAlteredTerms() { this.alteredTerms.clear(); }
    
    public void setTerm(ComparableTerm term, int index) throws org.biojava.ontology.AlreadyExistsException, IllegalArgumentException, ChangeVetoException {
        PersistentComparableTerm pt = (PersistentComparableTerm)this.db.convert(term);
        super.setTerm(pt, index);
