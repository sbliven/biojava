...
public class SimpleBioEntry extends AbstractChangeable implements BioEntry {
    ...
    /**
     * Overwrites the list of relationships at the given index position with the relationship
     * supplied. It will overwrite anything already at that position.
     * @param relationship New relationship to write at that position.
     * @param index Position to write relationship at.
     * @throws org.biojava.utils.ChangeVetoException in case of objections.
    ...
    public void setBioEntryRelationship(BioEntryRelationship relationship, int index) throws AlreadyExistsException,ChangeVetoException {
        if (this.relationships.contains(relationship)) throw new AlreadyExistsException("Relationship has already been made");
        if(!this.hasListeners(BioEntry.RELATIONSHIP)) {
            this.relationships.ensureCapacity(index+1);
    ...
    /**
     * Removes the relationship at a given index. If the index position already had no
     * relationship associated, it returns false. Else, it returns true.
     * @return True if a relationship was found at that position and removed.
     * @param index the index position to remove the relationship from.
     * @throws org.biojava.utils.ChangeVetoException in case of objections.
     * @throws IndexOutOfBoundsException if the index position was invalid.
     */
    public boolean removeBioEntryRelationship(int index) throws IndexOutOfBoundsException,ChangeVetoException {
        if (this.relationships.get(index)==null) return false;
        else {
            if(!this.hasListeners(BioEntry.RELATIONSHIP)) {
                this.relationships.set(index,null);
            } else {
                ChangeEvent ce = new ChangeEvent(
                        this,
                        BioEntry.RELATIONSHIP,
    ...
    /**
     * Returns the relationship at a given index. If the index is valid but no relationship is
     * found at that position, it will return null. If the index is invalid,
     * an exception will be thrown.
     * @param index the index of the relationship to retrieve.
     * @return The relationship at that index position.
     * @throws IndexOutOfBoundsException if an invalid index is specified.
     */
    public BioEntryRelationship getBioEntryRelationship(int index) throws IndexOutOfBoundsException {
        return (BioEntryRelationship)this.relationships.get(index);
    }
    ...
    public int addBioEntryRelationship(BioEntryRelationship relationship) throws AlreadyExistsException,ChangeVetoException {
        ...
        int index = this.relationships.size();
        if(!this.hasListeners(BioEntry.RELATIONSHIP)) {
            this.relationships.ensureCapacity(index+1);
     ...
     */
    public List getBioEntryRelationships() {
    ...
    public int countBioEntryRelationships() {
        return this.countList(this.relationships);
    }
